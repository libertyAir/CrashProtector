...
@interface LAUnrecognizedSelectorSolveObject ()
@end

@implementation LAUnrecognizedSelectorSolveObject
...
id addMethod(id self, SEL _cmd) {
    NSLog(@"LACrashProtector: unrecognized selector: %@", NSStringFromSelector(_cmd));
    return 0;
}
...
+ (void)la_enableSelectorProtector {
    ...
    dispatch_once(&onceToken, ^{
        ...
        [object la_instanceSwizzleMethod:@selector(forwardingTargetForSelector:) replaceMethod:@selector(la_forwardingTargetForSelector:)];
    });
}

- (id)la_forwardingTargetForSelector:(SEL)aSelector {
    ...
    LAUnrecognizedSelectorSolveObject *solveObject = [LAUnrecognizedSelectorSolveObject new];
